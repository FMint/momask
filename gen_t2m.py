...
if __name__ == '__main__':
    ...
    for r in range(opt.repeat_times):
        ...
        for k, (caption, joint_data)  in enumerate(zip(captions, data)):
            ...
            # ik_save_path = pjoin(animation_path, f"last8_{opt.name}_{opt.res_name}_len%d_ik.mp4"%(m_length[k]))
            ik_save_path = pjoin(animation_path, f"best2_{opt.name}_{opt.res_name}_len%d_ik.mp4"%(m_length[k]))
